...
>
> 广州大学长期对口帮扶多所基础教育学校。我们愿意将智学伴作为帮扶体系中的技术工具，向帮扶学校免费部署并提供持续的技术支持。通过数学图形渲染引擎，学生在没有专业教师的情况下也能直观理解函数图像、几何关系等抽象概念——这在师资匮乏的学校中尤为重要。

**我们不是在等待"被推广"，我们准备好了"走出去"。**

这不是写在纸上的愿景。智学伴的零服务端架构意味着：任何学校只需一台能上网的电脑，就能获得完整的 AI 教学能力。Windows 桌面 EXE 版本更是为网络条件不佳的地区量身打造。我们将在项目后续阶段主动联系广州大学帮扶学校及大湾区社区图书馆，提供免费部署、操作培训和技术支持，让这个项目真正走进有需要的地方。
...
### 🛡️ 直面质疑：AI 讲错了怎么办？学生用来作弊怎么办？

每一个 AI 教育产品都必须回答这两个问题。我们不回避，主动阐述我们的风险控制机制。

#### 问题一：AI 会产生"幻觉"，讲错了怎么办？

AI 幻觉——模型自信地输出看似合理但事实错误的内容——是当前大语言模型无法根除的固有缺陷。智学伴从三个层面构建了多层防御体系：

**第一层：提示词工程内置事实约束。** 我们的 AI 提示词在关键概念上设置了明确的边界指令——例如数学公式的 LaTeX 语法必须正确、历史事件的时间节点不可随意编造、科学定义的表述需保持严谨。这无法完全杜绝幻觉，但显著降低了模型"自由发挥"的概率。

**第二层：数学渲染引擎作为"事实校验器"。** 这是我们最独特的一道防线。AI 生成的错误数学表达式（如语法错误、变量未定义等）无法被 Python + matplotlib 渲染引擎正确执行，渲染会直接失败并返回具体错误信息。这意味着——**AI 在数学图像上撒不了谎，因为谎言画不出图。** 错误会在渲染环节被自动拦截，并将详细信息反馈给 AI 进行修正。这一机制将黑盒的"AI 幻觉"转化为白盒的"语法校验"，是我们在理工科教学领域最硬核的准确性保障。

**第三层：以教促学的"反向验证"。** 以教促学环节要求学生向 AI 讲解自己的解题思路——如果 AI 此前的批改或讲解存在错误，学生在尝试阐释时往往会发现逻辑矛盾。AI 的追问本身也会进一步暴露知识链条中的漏洞。学生在"教"的过程中无形中执行了对 AI 输出的事实核查，形成了人机协同的闭环校验机制。

#### 问题二：学生用 AI 抄答案、代写作业怎么办？

这是所有 AI 教育工具面临的伦理拷问。我们的回答是：**智学伴从设计哲学层面就不是一个"代劳工具"，而是一个"逼迫输出"的平台。**

**核心机制：以教促学强迫主动输出。** 在智学伴的学习闭环中，学生无法通过"复制粘贴 AI 答案"完成任务——因为最终的通过条件不是"提交正确答案"，而是**向 AI 讲解清楚为什么是这个答案**。AI 扮演的是"学生"角色，会追问、质疑和验证。一个没有真正理解知识点的学生，无法通过这一关。直接把 AI 答案念一遍会被立即识别为"复述"而拒绝通过。**智学伴的 AI 不是帮助学生抄答案的"枪手"，而是追问到底的"苏格拉底"。**

**辅助机制：考试模式下的 AI 助手行为约束。** 在试卷考试模式中，悬浮 AI 助手自动切换为"禁答模式"——只解释概念含义，坚决不透露解题思路和答案。学生在考试中无法利用 AI 作弊，但仍可在遇到陌生概念时获得及时的释义帮助。

**设计理念总结：** AI 可以成为学生最耐心的导师、最严格的考官、最细致的批改者，但绝不能成为学生的替身。这是我们贯穿产品始终的伦理红线。评委老师如果担心学生用 AI 抄答案——我们邀请您亲自试用"以教促学"环节，看 AI 如何"拷问"一个没有真正理解的学生。
